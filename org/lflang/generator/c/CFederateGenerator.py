...
#  
class CFederateGenerator(object):
    ...
    def generateFederateNeighborStructure(cls, federate):
        ...
        code_ = CodeBuilder()
        code_.pr("\n".join([ "/**", "* Generated function that sends information about connections between this federate and", "* other federates where messages are routed through the RTI. Currently, this", "* only includes logical connections when the coordination is centralized. This", "* information is needed for the RTI to perform the centralized coordination.", "* @see MSG_TYPE_NEIGHBOR_STRUCTURE in net_common.h", "*/", "void send_neighbor_structure_to_RTI(int rti_socket) {")])
        code_.indent()
        ...
        #  RTI.
        code_.pr("\n".join([ "interval_t candidate_tmp;", "size_t buffer_size = 1 + 8 + ", "                " + federate.dependsOn.keySet().size() + " * ( sizeof(uint16_t) + sizeof(int64_t) ) +", "                " + federate.sendsTo.keySet().size() + " * sizeof(uint16_t);", "unsigned char buffer_to_send[buffer_size];", "", "size_t message_head = 0;", "buffer_to_send[message_head] = MSG_TYPE_NEIGHBOR_STRUCTURE;", "message_head++;", "encode_int32((int32_t)" + federate.dependsOn.keySet().size() + ", &(buffer_to_send[message_head]));", "message_head+=sizeof(int32_t);", "encode_int32((int32_t)" + federate.sendsTo.keySet().size() + ", &(buffer_to_send[message_head]));", "message_head+=sizeof(int32_t);")])
        if not federate.dependsOn.keySet().isEmpty():
            ...
            for upstreamFederate in federate.dependsOn.keySet():
                code_.pr("\n".join([ "encode_uint16((uint16_t)" + upstreamFederate.id + ", &(buffer_to_send[message_head]));", "message_head += sizeof(uint16_t);")])
                ...
                if delays != None:
                    ...
                    for delay in delays:
                        if delay == None:
                            ...
                            code_.pr("\n".join([ "if (" + delayTime + " < candidate_tmp) {", "    candidate_tmp = " + delayTime + ";", "}")])
                    code_.pr("\n".join([ "encode_int64((int64_t)candidate_tmp, &(buffer_to_send[message_head]));", "message_head += sizeof(int64_t);")])
                else:
                    #  Use NEVER to encode no delay at all.
                    code_.pr("\n".join([ "encode_int64(NEVER, &(buffer_to_send[message_head]));", "message_head += sizeof(int64_t);")])
        ...
        if not federate.sendsTo.keySet().isEmpty():
            ...
            for downstreamFederate in federate.sendsTo.keySet():
                code_.pr("\n".join([ "encode_uint16(" + downstreamFederate.id + ", &(buffer_to_send[message_head]));", "message_head += sizeof(uint16_t);")])
        code_.pr("\n".join([ "write_to_socket_errexit(", "    rti_socket, ", "    buffer_size,", "    buffer_to_send,", "    \"Failed to send the neighbor structure message to the RTI.\"", ");")])
        code_.unindent()
        code_.pr("}")
